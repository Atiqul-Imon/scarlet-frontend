...
export async function POST(request: Request) {
  try {
    ...
    console.log('SSLCommerz Failed Callback (POST) received:', data);
    ...
    return NextResponse.redirect(new URL(`/payment/failed?${queryParams.toString()}`, request.url));

  } catch (error) {
    console.error('Error handling SSLCommerz failed callback:', error);
    // Redirect to payment failed page with generic error message
    return NextResponse.redirect(new URL('/payment/failed?message=Payment processing error', request.url));
  }
}
