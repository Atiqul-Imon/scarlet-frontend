...
export async function GET(request: Request) {
  try {
    ...
    logger.info('SSLCommerz Cancelled Callback (GET) received', { hasData: Object.keys(data).length > 0 });
...
export async function POST(request: Request) {
  try {
    const data = await extractCallbackData(request);
    logger.info('SSLCommerz Cancelled Callback (POST) received', { hasData: Object.keys(data).length > 0 });
