...
export async function POST(request: Request) {
  try {
    console.log('POST request received to /payment/success');
    ...
    const redirectUrl = new URL(`/payment/success?${queryParams.toString()}`, baseUrl);
    ...
    const baseUrl = new URL(request.url).origin;
    return NextResponse.redirect(new URL('/payment/success?error=processing_error', baseUrl));
  }
}
