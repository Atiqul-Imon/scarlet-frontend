...
export async function POST(request: Request) {
  try {
    console.log('POST request received to /payment/cancelled');
    ...
    const redirectUrl = new URL(`/payment/cancelled?${queryParams.toString()}`, baseUrl);
    ...
    const baseUrl = new URL(request.url).origin;
    return NextResponse.redirect(new URL('/payment/cancelled?error=processing_error', baseUrl));
  }
}
